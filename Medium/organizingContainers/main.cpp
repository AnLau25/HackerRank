...
string organizingContainers(vector<vector<int>> container) {
    ...
    vector<long long> container_capacity(n, 0);

    vector<long long> ball_type_count(n, 0);
    ...
    sort(ball_type_count.begin(), ball_type_count.end());

    if (container_capacity == ball_type_count)
        return "Possible";
    else
        return "Impossible";
}
