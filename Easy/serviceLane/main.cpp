...
vector<int> serviceLane(int n, vector<vector<int>> cases, vector<int> width) {
    vector<int> solns(cases.size());
    
     for (int i = 0; i < cases.size(); i++) {
        auto start_iterator = width.begin() + cases[i][0];
        auto end_iterator   = width.begin() + cases[i][1] + 1;

        int minVal = *min_element(start_iterator, end_iterator);
        solns[i] = minVal;
    }
...
Input format:
 1. 8 5                 <n = 8, t = 5>
 2. 2 3 1 2 3 2 3 3     <width = [2, 3, 1, 2, 3, 2, 3, 3]>
 3. 0 3                 <cases = [[0, 3], [4, 6], [6, 7], [3, 5], [0, 7]]>
 4. 4 6
 5. 6 7
 6. 3 5
 7. 0 7
